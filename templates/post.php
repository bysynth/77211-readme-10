                    ...
                    <div class="post__buttons">
                        <?php if (isset($post['id'])): ?>
                            <a class="post__indicator post__indicator--likes button"
                               href="/like.php?post_id=<?= clear_input($post['id']) ?>" title="Лайк">
                        ...
                        <?php if (isset($post['id'])): ?>
                            <a class="post__indicator post__indicator--repost button"
                               href="/repost.php?post_id=<?= clear_input($post['id']) ?>" title="Репост">
                                <svg class="post__indicator-icon" width="19" height="17">
                    ...
                    <form class="comments__form form" method="post">
                        <?php if (isset($_SESSION['user']['avatar'])): ?>
                            <div class="comments__my-avatar">
                                <img class="comments__picture" src="/uploads/<?= clear_input($_SESSION['user']['avatar']) ?>" alt="Аватар пользователя">
                            </div>
                        <?php endif ?>
                        <div class="form__input-section <?= !empty($error) ? 'form__input-section--error' : '' ?>">
                            <textarea class="comments__textarea form__textarea form__input"
                                      placeholder="Ваш комментарий" name="comment" id="comment"><?= clear_input(get_post_val('comment')) ?></textarea>
                            <label for="comment" class="visually-hidden">Ваш комментарий</label>
                            <input type="hidden" name="post-id" value="<?= clear_input($post['id']) ?>">
                                ...
                                <li class="comments__item user">
                                    <div class="comments__avatar">
                                        <?php if (isset($comment['author_id'], $comment['avatar'])): ?>
                                            <a class="user__avatar-link" href="<?= '/profile.php?user=' . clear_input($comment['author_id']) ?>">
                                    ...
                                    <div class="comments__info">
                                        <div class="comments__name-wrapper">
                                            <?php if (isset($comment['name'], $comment['author_id'])): ?>
                                                <a class="comments__user-name" href="<?= '/profile.php?user=' . clear_input($comment['author_id']) ?>">
                ...
                <div class="post-details__user-info user__info">
                    <div class="post-details__avatar user__avatar">
                        <?php if (isset($post['author_id'])): ?>
                            <a class="post-details__avatar-link user__avatar-link" href="<?= '/profile.php?user=' . clear_input($post['author_id']) ?>">
                    ...
                    <div class="post-details__name-wrapper user__name-wrapper">
                        <?php if (isset($post['author_id'])): ?>
                            <a class="post-details__name user__name"
                               href="<?= '/profile.php?user=' . clear_input($post['author_id']) ?>">
                ...
                <?php if ($session_user_id !== $post['author_id']): ?>
                    <div class="post-details__user-buttons user__buttons">
                        <a class="user__button user__button--subscription button button--main"
                           href="/subscribe.php?subscribe_user_id=<?= clear_input($post['author_id']) ?? '' ?>">
                            <?= $is_subscribed ? 'Отписаться' : 'Подписаться' ?></a>
                        <?php if ($is_subscribed): ?>
                            <a class="user__button user__button--writing button button--green"
                               href="/messages.php?user_id=<?= clear_input($post['author_id']) ?>">Сообщение</a>
                        <?php endif; ?>
                    </div>
                <?php endif ?>
            </div>
