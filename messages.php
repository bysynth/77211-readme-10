...
$contact_id = get_contact_id($db_connect, $session_user, $interlocutor_id);
...
if ($contact_id === null) {
    ...
    $sql_insert_contacts = "INSERT INTO contacts (sender_id, receiver_id) VALUES 
                            ($session_user, $interlocutor_id),
                            ($interlocutor_id, $session_user)";
...
$sql_interlocutors = "SELECT u.id, u.name, u.avatar, 
                (SELECT m.created_at
                 ...
                 WHERE (c1.sender_id = $session_user AND c1.receiver_id = u.id) 
                 OR (c2.sender_id = u.id AND c2.receiver_id = $session_user)
                 ORDER BY m.created_at DESC LIMIT 1) AS last_message_date
                FROM contacts AS c
                JOIN users AS u ON c.receiver_id = u.id
                WHERE c.sender_id = $session_user";
...
if (isset($interlocutor_id)) {
    ...
    $sql_messages = "SELECT u.id, u.name, u.avatar, m.created_at, m.message
                    ...
                    WHERE (c1.sender_id = $session_user AND c1.receiver_id = $interlocutor_id) 
                    OR (c2.sender_id = $interlocutor_id AND c2.receiver_id = $session_user)
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $error = validate_message($db_connect, $message_input, $receiver_id, $session_user);
